...
df.AllSamples2plot <- df.AllSamples %>% filter(sample %in% c("2021EE01",
                                                             "2021EE05", "2021EE06", "2021EE07", 
                                                             "2021EE08", "2021EE16", "2021EE18"))

## including T20.LB.1
df.AllSamples2plot <- df.AllSamples %>% filter(sample %in% c("2021EE01",
                                                             "2021EE05", "2021EE06", "2021EE07", 
                                                             "2021EE08", "2021EE13", "2021EE18"))
...
df.AllSamples2plot$y.order <- factor(df.AllSamples2plot$y.order, levels = c("1"))
...
df.AllSamples2plot %>% 
  filter(sample == "2021EE13" & window.end < 40000 & chr == "USMA_521_v2_9") %>% 
  reframe(mean = mean(log2ratio))
  
  
  
plot.chr9.Figure3.USMA_Paper <- df.AllSamples2plot %>% 
  filter(sample != "2021EE01") %>% 
...
##
# Figure.A <- df.AllSamples2plot %>% 
Figure.A <- df.AllSamples2plot %>% 
  filter(sample != "2021EE01" & chr == "USMA_521_v2_9") %>% 
  filter(Group == "H2O2_Shock") %>% 
  ggplot()+
  geom_hline(yintercept = 0, color = "black", linewidth = 0.4)+
  geom_hline(yintercept = 0.5, linetype = "dotted", color = "gray", linewidth = 0.2)+
  geom_hline(yintercept = 1, linetype = "dashed", color = "gray", linewidth = 0.4)+
  geom_hline(yintercept = 1.5, linetype = "dotted", color = "gray", linewidth = 0.2)+
  geom_hline(yintercept = 2, linetype = "dashed", color = "gray", linewidth = 0.4)+
  geom_line(aes(x = window.end/1000, y = log2ratio),  color = "darkred", linewidth = 0.2) +
  ...
  theme(legend.position = "none",
        ...
        panel.spacing = unit(0.9, "cm", data = NULL))
  
Figure.A
# Edit Figure 4 A
Figure.A <- ggplotGrob(Figure.A)

lg <- linesGrob(x=unit(c(0,0),"npc"), y=unit(c(0,1),"npc"), 
                gp=gpar(col="black", lwd=4))

for (k in grep("strip-r", Figure.A$layout$name)) {
  Figure.A$grobs[[k]]$grobs[[1]]$children[[1]] <- lg
}
...
# Figure B
Figure.B <- df.AllSamples2plot %>% 
  filter(sample != "2021EE01" & chr == "USMA_521_v2_9") %>% 
  filter(Group == "No_H2O2") %>% 
  ggplot()+
  geom_hline(yintercept = 0, color = "black", linewidth = 0.4)+
  geom_hline(yintercept = 0.5, linetype = "dotted", color = "gray", linewidth = 0.2)+
  geom_hline(yintercept = 1, linetype = "dashed", color = "gray", linewidth = 0.4)+
  geom_hline(yintercept = 1.5, linetype = "dotted", color = "gray", linewidth = 0.2)+
  geom_hline(yintercept = 2, linetype = "dashed", color = "gray", linewidth = 0.4)+
  geom_line(aes(x = window.end/1000, y = log2ratio),  color = "darkblue", linewidth = 0.2) +
  facet_grid(rows = vars(Line))+
  scale_x_continuous(limits = c(0, max.chr9/1000),
                     breaks = c(seq(0, max.chr9/1000, 150), 
                                (max.chr9/1000)))+ 
...
# Edit Figure 4 B
Figure.B <- ggplotGrob(Figure.B)

lg <- linesGrob(x=unit(c(0,0),"npc"), y=unit(c(0,1),"npc"), 
                gp=gpar(col="black", lwd=4))

for (k in grep("strip-r", Figure.B$layout$name)) {
  Figure.B$grobs[[k]]$grobs[[1]]$children[[1]] <- lg
}
...
Figura.AB <- plot_grid(Figure.A, Figure.B, ncol = 2, align = "v", scale = 0.96, labels = c("A)", "B)"))

# Export the plot to a PNG file using Cairo

## New figure
dir.create("ColonyAnalysis")

Figure3.Paper <- paste0("ColonyAnalysis/Figure3_USMA_Paper.v1.png")

Cairo(file = Figure3.Paper, type = "png", width = 8, height = 4, units = "in", dpi = 300)
print(Figura.AB)
dev.off()
getwd()
...
# 2023.10.06
#for supp figure
df.AllSamples2plot %>% filter(sample %in% c( "2021EE37"))
